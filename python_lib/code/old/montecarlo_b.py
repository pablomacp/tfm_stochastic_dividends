...
def HybridStockDividendsMSamples(S0,q0,r,a_pasos,b_pasos,volS_pasos,volq_pasos,rho,M,N,h,mat_pos):
    # M number of paths.
    ...
    for days_year in mats:
        if inicio == 0: # Este if sirve para resetear a 0 el indice de dividendos cada año.
            ...
            q[inicio + 1] = q[inicio] + h[inicio] * (a[inicio] * np.ones(2 * M) - b[inicio] * q[inicio]) + np.multiply(
        ...
        for dia in range(inicio + 1, days_year):
            ...
            q[dia + 1] = q[dia] + h[dia] * (a[dia] * np.ones(2 * M) - b[dia] * q[dia]) + np.multiply(
